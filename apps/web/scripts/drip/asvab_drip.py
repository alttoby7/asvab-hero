...
signup. If today matches a drip day (2, 5, 7, 10, 14, 21, 30, 45, 60, 90) AND the day has not yet
...
try:
    from cron_status import report as report_cron_status
except ImportError:
    def report_cron_status(*a, **k):
        return False

DRIP_SCHEDULE = {
    2: 7,    # Day 2: WK mistakes
    5: 8,    # Day 5: GT math (Pass-led CTA)
    7: 17,   # Day 7: Pro upgrade
    10: 9,   # Day 10: hidden jobs
    14: 10,  # Day 14: retake (Pass-led CTA)
    # Evergreen extension 2026-07-18 (day-14 cliff fix). New signups flow through
    # these; existing subs past each day were backfilled into drip_sent
    # (backfill_drip_guard.py) so they are NOT retroactively blasted.
    21: 23,  # Day 21: AR is the biggest AFQT lever
    30: 24,  # Day 30: final 2-week plan
    45: 25,  # Day 45: PC quick wins
    60: 26,  # Day 60: line scores pick your job
    90: 27,  # Day 90: break the plateau
}
...
def main():
    ...
    if not DRY_RUN:
        report_cron_status("asvab_drip", "ok", {"sent_today": sent_count, "skipped": skipped_count, "excluded": excluded_count})
