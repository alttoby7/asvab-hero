-- 0043_official_status_requires_afqt.sql
-- Fix: official_test_status='taken_logged' must mean a USABLE official AFQT was
-- logged, not merely that a real_asvab_tests row exists.
--
-- Background: 0042's backfill (and rpc_log_official_test) set 'taken_logged'
-- whenever a real_asvab_tests row existed — even an INCOMPLETE one with afqt NULL.
-- That wrongly suppressed the "log your official scores" prompt and showed no
-- official anchor on the dashboard. Here we (1) correct the existing data and
-- (2) patch the RPC so the status reflects whether an AFQT was actually captured.

begin;

-- =============================================================
-- (1) Correct rows wrongly marked 'taken_logged' without a usable AFQT.
--     They have a real test on record but no logged AFQT → 'taken_not_logged'.
--     (Conservative: only touches the wrongly-set rows; leaves legitimate
--      'not_taken' / 'taken_not_logged' / properly-logged rows alone.)
-- =============================================================
update profiles p
  set official_test_status = 'taken_not_logged',
      updated_at = now()
  where p.official_test_status = 'taken_logged'
    and not exists (
      select 1 from real_asvab_tests r
      where r.user_id = p.user_id and r.afqt is not null
    );

-- =============================================================
-- (2) Patch rpc_log_official_test: set 'taken_logged' only when a usable AFQT
--     was provided; otherwise the user took a test but logged no AFQT.
--     Identical to 0042 except the status-setting block.
-- =============================================================
...
begin
  ...
  -- Status reflects whether a USABLE official AFQT was actually captured.
  update profiles
    set official_test_status =
          case when p_afqt is not null then 'taken_logged' else 'taken_not_logged' end,
        updated_at = now()
    where user_id = v_uid;
